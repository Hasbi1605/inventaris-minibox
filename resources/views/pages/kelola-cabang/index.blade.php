                ...
                <div class="flex items-center">
                    ...
                    @if(request('search') || request('kategori_id') || request('status'))
                        <span class="ml-3 px-2 py-1 text-xs font-semibold text-white bg-blue-600 rounded-full">
                            {{ collect([request('search'), request('kategori_id'), request('status')])->filter()->count() }} aktif
                        </span>
                    @endif
                </div>
        ...
        <div id="filter-content" class="hidden border-t border-gray-200">
            ...
            <form method="GET" action="{{ route('kelola-cabang.index') }}">
                ...
                <div class="flex justify-end mt-4 space-x-2">
                    <a href="{{ route('kelola-cabang.index') }}" class="inline-block px-6 py-2 font-bold text-center text-slate-700 uppercase align-middle transition-all rounded-lg cursor-pointer bg-gradient-to-tl from-gray-100 to-gray-200 leading-pro text-xs ease-soft-in tracking-tight-soft shadow-soft-md bg-150 bg-x-25 hover:scale-102 active:opacity-85 hover:shadow-soft-xs whitespace-nowrap">
