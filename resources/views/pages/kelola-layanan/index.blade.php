...
<div class="w-full max-w-full min-h-screen">
            ...
            <div class="p-6">
                <form method="GET" action="{{ route('kelola-layanan.index') }}">
                    <div class="flex items-end gap-3">
                        ...
                        <div class="flex-1">
                            <label class="inline-block mb-2 ml-1 font-bold text-xs text-slate-700">
                                <i class="fas fa-search mr-1"></i>Cari Layanan
                            </label>
                            <input type="text" name="search" value="{{ request('search') }}" placeholder="Nama layanan..." class="focus:shadow-soft-primary-outline text-sm leading-5.6 ease-soft block w-full appearance-none rounded-lg border border-solid border-gray-300 bg-white bg-clip-padding px-3 py-2 font-normal text-gray-700 transition-all focus:border-fuchsia-300 focus:outline-none focus:transition-shadow">
                        </div>

                        <!-- Buttons -->
                        <div class="flex gap-2">
                            <a href="{{ route('kelola-layanan.index') }}" class="inline-block px-6 py-2 font-bold text-center text-slate-700 uppercase align-middle transition-all rounded-lg cursor-pointer bg-gradient-to-tl from-gray-100 to-gray-200 leading-pro text-xs ease-soft-in tracking-tight-soft shadow-soft-md bg-150 bg-x-25 hover:scale-102 active:opacity-85 hover:shadow-soft-xs whitespace-nowrap">
    ...
    @include('pages.kelola-layanan.partials.tab-semua-cabang', [
    ...
    @foreach($cabangList as $cabang)
        @include('pages.kelola-layanan.partials.tab-per-cabang', [
            'cabangId' => $cabang->id,
            'cabangNama' => $cabang->nama_cabang,
            'layanan' => $layananPerCabang[$cabang->id],
